...
class BookLatestScreen extends StatefulWidget {
  const BookLatestScreen({super.key});

  @override
  State<BookLatestScreen> createState() => _BookLatestScreenState();
}

class _BookLatestScreenState extends State<BookLatestScreen> {
  late EbookBloc bloc;
  @override
  void initState() {
    bloc = EbookBloc();
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData) {
            final books = snapshot.data!
                .where((item) => item.latest.toString().contains('3'))
                .toList();
